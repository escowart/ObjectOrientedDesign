...
public abstract class RBTree<T> implements Iterable<T> {
    ...
    /** Modify the First Bit List by adding the Seconds Values to it.
     *  A carry over the 32 bit limit is added to the 0th element
     *  @param first : An int[] of 0s and 1s of size 32
     *  @param second : An int[] of 0s and 1s of size 32
     */
    protected static void bitListAddSecToFirst(int[] first, int[] second) {
        int c = 0;
        int i;
        for (i = 0; i < 32; i++) {
            first[i] += second[i] + c;
        ...
        // Condition for if carry from the last bit
        i = 0;
        while (c != 0) {
            first[i] += c;
            if (first[i] == 2) {
    ...
    /** Convert the given Bit Array to its corresponding int
     * @param blist : An int[] of 0s and 1s of size 32
     * @return : An int equal to the blist */
    protected static int bitListToInt(int[] blist) {
        double acc = 0;
        for (int i = 0; i < 32; i++) {
            acc += blist[i] * Math.pow(2, i);
        }
        return (int)(acc - Math.pow(2, 31));
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof RBTree) {
            ...
            if (this.size == bt.size) {
                Stack<T> s0 = this.toStack();
                Stack<?> s1 = bt.toStack();
                for (int i = 0; i < this.size; i++) {
                    if (!s0.pop().equals(s1.pop())) {
                        return false;
                    }
                }
                return true;
    ...
    /** Get the swap Array of ints which has a length of 32 and 
     *   contains one instance of each number between 0 & 31 
     *   based on the given seed
     * Functionality :
     * @param s : An int >= 0
     * @return : An int[] of size 32 of one instances 
     *   of each int between 0 & 31
     *  +aval - Adds the second number to the first number one at a time and 
     *   checks whether the number produced after every addition is already
     *   in the result list. if it is then it keeps adding 1 and modding by 32 
     *   until it is no longer in the list.
     *  0th element = s % 32
     *  1st element = (0th +aval (1 + s % 31)) % 32
     *  nth element = (n-1th +aval (1 + s % (32 - n))) % 32 
     *  */
    protected static int[] getSwapList(int s) {
        ...
        for (int i = 31; i > 0; i--) {
            n = (s % i);
            int t = (hits.get(hits.size() - 1) + 1) % 32;
            while (n > 0) {
                t = (t + 1) % 32;
                if (!hits.contains(t)) {
                    n--;
     ...
     */
    public int hashCode() {
        int[] ilist = new int[32];
        Arrays.fill(ilist, 0);
        for (int i = 0; i < this.size; i++) {
            RBTree.bitListAddSecToFirst(ilist, 
                    RBTree.reorderBitList(i,
                            RBTree.intToBitList(this.get(i).hashCode())));
        }
        return RBTree.bitListToInt(ilist);
    }
    ...
    /** Convert the given int + 2^31 to an Array of Bits 
     *   with the bit in the nth element in the array 
     *   being the value at the 2^n position
     * @param i : An int
     * @return : An int[] of 0s and 1s of size 32 which is equal to the int
     */
    protected static int[] intToBitList(int i) {
        int[] iarray = new int[32];
        long n = i + (long)Math.pow(2, 31);
        for (int j = 0; j < 32; j++) {
            iarray[j] = (int)(n % 2);
            n /= 2;
        }
    ...
    /** Reorder the given Bit Array with the given seed
     * @param s : An int that acts as the seed for the algorithm to reorder
     * @param barray : An int[] of 0s and 1s of size 32 
     * @return : An int[] of 0s and 1s of size 32 
     *   which is the reordered Bit Array
     */
    protected static int[] reorderBitList(int s, int[] barray) {
        int[] swaplist = RBTree.getSwapList(s);
        int[] modlist = new int[32];
        for (int i = 0; i < 32; i++) {
            modlist[i] = barray[swaplist[i]];
        }
        return modlist;
    }
    ...
    /** Convert this Red-Black Tree into a Stack
     * @return : A Stack<T> of the this Red-Black Tree Flattened
     */
    protected Stack<T> toStack() {
        return this.toStack(new Stack<T>());
    }
    
    /** Stack all elements in this Red-Black Tree onto the given Stack
     * @param s : A Stack<T>
     * @return : A Stack<T> of the this Red-Black Tree Flattened 
     *  onto the given Stack<T>
     */
    protected Stack<T> toStack(Stack<T> s) {
        if (!this.isEmpty()) {
            if (this.hasRight()) {
                s = this.right.toStack(s);
            }
            s.push(this.val);
            if (this.hasLeft()) {
                s = this.left.toStack(s);
            }
        }
        return s;
    }
